...
class AttentionLS(nn.Module):
    ...
    def forward(self, X, mask, cls_embed=None):
        ...
        dct_scalar = 1e2
#         Q = self.split_heads(dct.dct_real(dct.dct_real(self.W_q(X),dim=2)/dct_scalar,dim=1)/dct_scalar/math.sqrt(self.head_dim))
            
#         K = self.split_heads(self.dual_ln_l(dct.dct_real(dct.dct_real(self.W_k(X),dim=2)/dct_scalar,dim=1)/dct_scalar))
#         V = self.split_/heads(self.dual_ln_l(dct.dct_real(dct.dct_real(self.W_v(X),dim=2)/dct_scalar,dim=1)/dct_scalar))
...
#         C = dct.idct_real(dct.idct_real(C*dct_scalar,dim = 1)*dct_scalar,dim = 2)
