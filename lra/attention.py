...
class Attention(nn.Module):
    ...
    def forward(self, X, mask, cls_embed=None):
        ...
        if self.attn_type.startswith("longformer") or self.attn_type.startswith("reformer"):
            ...
            with torch.cuda.amp.autocast(enabled = False):
                if self.grad_checkpointing:
                    attn_out = checkpoint(self.attn,(self.split_heads(X)).float(), mask.float(),cls_embed = cls_embed)
                else:
                    attn_out = self.attn((self.split_heads(X)).float(),mask.float(),cls_embed = cls_embed)
        ...
        # out = dct.idct_real(out,dim = 1)
        # out = m.permute(0, 2, 1).float()
        return out
