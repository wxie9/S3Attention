...
class LinformerAttention(nn.Module):
    ...
    def forward(self, Q, K, V, mask, dconv_weight=None):
        ...
        attn = nn.functional.softmax(dot, dim = -1) #+ torch.transpose(nn.functional.softmax(torch.transpose(dot,-2,-1),dim=-1),-2,-1)

        KK = KK * mask[:, None, :, None]
        VV = VV * mask[:,None, :, None]
        dot1 = torch.matmul( torch.transpose(QQ, -2, -1),KK)
        dot1 = dot1 / math.sqrt(self.seq_len)
        attn1 = torch.softmax(dot1,dim=-1)#.transpose(-2,-1)
       

        X = torch.matmul(attn, V) +  torch.matmul(VV, attn1)
